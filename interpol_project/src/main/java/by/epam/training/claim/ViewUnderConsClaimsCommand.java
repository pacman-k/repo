...
import static by.epam.training.ApplicationConstants.VIEW_UNDER_CONS_CLAIMS_CMD_NAME;

@Log4j
@Bean(name = VIEW_UNDER_CONS_CLAIMS_CMD_NAME)
@AllArgsConstructor
@Getter
public class ViewUnderConsClaimsCommand implements ServletCommand {
    ClaimService claimService;
    StatusService statusService;

    @Override
    public void execute(HttpServletRequest req, HttpServletResponse resp) throws CommandException {
        ClaimStatusDto status = statusService.getStatusByName(StatusTypes.UNDER_CONSIDERATION.toString()).orElseThrow(
                ()-> new CommandException("cant find claim under consideration claim status"));
